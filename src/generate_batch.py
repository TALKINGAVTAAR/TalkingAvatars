...
def get_data(first_coeff_path, audio_path, device, ref_eyeblink_coeff_path, still=False, idlemode=False, length_of_audio=False, use_blink=True):
    ...
    # Construct the correct path to the `.mat` file
    input_dir = os.path.dirname(first_coeff_path)  # Get directory of first_coeff_path
    source_semantics_path = os.path.join(input_dir, 'Umesh Own AI Host Avtaar.mat')  # Directly join to the .mat file name

    # Ensure we are using an absolute path
    source_semantics_path = os.path.abspath(source_semantics_path)

    # Check if file exists before loading to avoid FileNotFoundError
    if not os.path.exists(source_semantics_path):
        raise FileNotFoundError(f"The file {source_semantics_path} does not exist.")

    # Now load the file using the dynamically constructed path
